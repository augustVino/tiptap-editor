  ...
  return (
    <section>
      ...
      <div style={{ marginBottom: 12, display: 'flex', gap: 8 }}>
        ...
        <button onClick={() => editorRef.current?.setContent('<p>通过 ref 设置的内容</p>')}>
          设置内容
        </button>
        <button onClick={() => editorRef.current?.insertContent('<strong>插入的粗体</strong>')}>
          插入内容
        </button>
      ...
      <details style={{ marginTop: 12 }}>
        <summary><strong>实时内容 (HTML)</strong></summary>
        <pre style={preStyle}>{content}</pre>
      </details>

      <details style={{ marginTop: 8 }}>
        <summary><strong>通过 ref 获取的内容</strong></summary>
        <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
          <button onClick={() => alert(editorRef.current?.getHTML())}>getHTML()</button>
          <button onClick={() => alert(editorRef.current?.getText())}>getText()</button>
          <button onClick={() => alert(JSON.stringify(editorRef.current?.getJSON(), null, 2))}>
            getJSON()
          </button>
          <button onClick={() => alert(String(editorRef.current?.isEmpty()))}>isEmpty()</button>
        </div>
      </details>
    </section>
  );
